...
// 精准扶贫
@property (nonatomic, strong) TBPovertyAlleviationView *povertyAlleviationView;
//尾部视图
...
- (void)setScrollViewContent
{
    ...
    self.povertyAlleviationView = [[TBPovertyAlleviationView alloc] init];
    [self.scrollView addSubview:self.povertyAlleviationView];
    [viewArray addObject:self.povertyAlleviationView];
...
- (void)valuesViewTabelIndex:(NSInteger)index
{
    ...
    {
        [self.povertyAlleviationView updataMaking];
    }
    else if (index == 8)
    {
...
- (void)assignmentData:(TBMakingListMode *)mode
{
    ...
    [self.povertyAlleviationView updataData:self.makingList];
}
