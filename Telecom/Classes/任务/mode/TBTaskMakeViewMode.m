...
- (void)authenticationInformation
{
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        if (audioDataUrl == nil || bossHeadImageUrl == nil || appearanceImageUrls == nil || foodImagesUrls == nil || environmentImagesUrls == nil ||coverImageUrl == nil||povertyImages.count !=weakSelf.makingMode.povertyPhotoArray.count)
        {
            if (self.hiddeHUD == NO)
            {
                [weakSelf hudDism];
...
- (void)submitServiceSuccessful:(successful)successful failure:(failure)failure;
{
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        if (images == nil||povertyImages.count !=weakSelf.makingMode.povertyPhotoArray.count)
        {
            if (self.hiddeHUD == NO)
            {
                [weakSelf hudDism];
...
- (void)isNextStepIndex:(NSInteger)row state:(void(^)(BOOL successful))state;
{
    ...
    {
        if (self.makingMode.povertyMsg.length >0)
        {
            [UIView addMJNotifierWithText:self.makingMode.povertyMsg dismissAutomatically:YES];
        }
        state(self.makingMode.povertyMsg.length == 0);
    }
    else if (row == 8)
    {
        state(YES);
